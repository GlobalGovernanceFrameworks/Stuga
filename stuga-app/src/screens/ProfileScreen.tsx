...
export default function ProfileScreen({ navigation }: any) {
  ...
  const [isBlockCaptain, setIsBlockCaptain] = useState(false);
  const [blockCaptainPhone, setBlockCaptainPhone] = useState('');
  ...
  async function loadProfile() {
    try {
      ...
      if (userDoc.exists()) {
        ...
        setIsBlockCaptain(userData.is_block_captain || false);
        setBlockCaptainPhone(userData.block_captain_phone || '08-590 970 00');
      }
      ...
      showSnackbar('⚠️ Kunde inte ladda profil. Försök igen.', 5000);
      setLoading(false);
    }
  }
  ...
  async function handleSaveProfile() {
    ...
    try {
      ...
      await updateDoc(doc(db, 'users', currentUser.uid), {
        ...
        is_block_captain: isBlockCaptain,
        block_captain_phone: isBlockCaptain ? blockCaptainPhone : null
      });
      ...
      showSnackbar('⚠️ Kunde inte spara profil. Försök igen.', 5000);
    } finally {
      setSaving(false);
    }
  }
  ...
  async function loadStats(userId: string) {
    try {
      ...
      // Silent fail - stats är inte kritiskt
      // Användaren ser fortfarande profilen
    }
  }
  ...
  async function loadRecentTransactions(userId: string) {
    try {
      ...
      // Silent fail - transaktioner är inte kritiskt för profil
    }
  }
  ...
  return (
    ...
    >
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Text style={styles.sectionTitle}>🛡️ Kvartersvärd</Text>
          
          <View style={styles.settingRow}>
            <View style={styles.settingInfo}>
              <Text style={styles.settingLabel}>Jag vill vara kvartersvärd</Text>
              <Text style={styles.settingDescription}>
                Du blir en lokal kontaktperson mellan grannar och kommunen vid kris
              </Text>
            </View>
            <Switch
              value={isBlockCaptain}
              onValueChange={async (value) => {
                setIsBlockCaptain(value);
                const currentUser = auth.currentUser;
                if (currentUser) {
                  await updateDoc(doc(db, 'users', currentUser.uid), {
                    is_block_captain: value,
                    block_captain_phone: value ? (blockCaptainPhone || '08-590 970 00') : null
                  });
                }
              }}
              color="#2D5016"
          ...
          {isBlockCaptain && (
            <>
              <Divider style={{ marginVertical: 16 }} />
              <Text style={styles.settingLabel}>Kommunens telefonnummer</Text>
              <TextInput
                value={blockCaptainPhone}
                onChangeText={setBlockCaptainPhone}
                mode="outlined"
                style={styles.input}
                keyboardType="phone-pad"
                placeholder="08-590 970 00"
              />
              <Text style={styles.settingDescription}>
                Detta nummer sparas för snabb kontakt vid kris
              </Text>

              <Button
                mode="contained"
                onPress={() => navigation.navigate('BlockCaptainGuide')}
                style={{ marginTop: 16 }}
                buttonColor="#2D5016"
                icon="shield-check"
              >
                Visa kvartersvärd-guide
              </Button>
            </>
          )}
        </Card.Content>
